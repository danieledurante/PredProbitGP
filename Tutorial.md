...
Recalling Section 3, to generate the Gaussian process we first create `10000` **training locations** on a 100 × 100 grid in the unit square.
...
Once this has been done, we produce `100` test locations on a 10 × 10 **grid in the unit square**, and another `100` **randomly distributed in the unit square**. These locations are required to assess **out-of-sample test performance** of the different methods under analysis.
...
Leveraging the above function, we can **simulate the GP realizations at both training and test locations**, and then **transform these values into probabilities using the probit link**, as described in Section 3 of the article.
