...
```{r}
mle_func_TLR <- function(geom, y, alpha)
{
  n <- nrow(geom)
  xi <- rep(0, n)
  geom[, 1] <- geom[, 1] * alpha[1]
  ...
  return(tlrmvnmvt::pmvn(lower = rep(-Inf, n), upper = xi, sigma = covM, uselog2 = T, N = 998, 
                             algorithm = TLRQMC(m = round(sqrt(nrow(covM))), epsl = 1e-4)))
}
mle_func_TN <- function(geom, y, alpha)
{
  n <- nrow(geom)
  xi <- rep(0, n)
  geom[, 1] <- geom[, 1] * alpha[1]
  ...
  covM <- outer(2 * y - 1, 2 * y - 1) * covM
  diag(covM) <- diag(covM) + 1
  return(TruncatedNormal::pmvnorm(mu = rep(0, length(xi)), sigma = covM, 
                                  log = TRUE, ub = xi)[[1]])
}
```
